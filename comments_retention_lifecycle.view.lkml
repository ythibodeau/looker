view: comments_retention_lifecycle {
  derived_table: {
    sql_trigger_value: SELECT CURDATE() ;;
    sql: SELECT pdt5.yearmonth as yearmonth
          , pdt5.account_id
          , pdt4.monthly_comments
          , pdt4.previous_comment_month
        FROM ${year_month_accounts.SQL_TABLE_NAME} as pdt5
        LEFT OUTER JOIN ${monthly_activity_previous_comments.SQL_TABLE_NAME} as pdt4
...
#   }
  ...
  dimension: is_new {
    type: yesno
    sql: ${monthly_comments} > 0 AND ${previous_comment_month} IS NULL ;;
  }
  ...
  set: detail {
    fields: [yearmonth, account_id, monthly_comments, previous_comment_month]
  }
}
