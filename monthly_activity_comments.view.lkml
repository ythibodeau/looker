view: monthly_activity_comments {
 derived_table: {
  ...
  SELECT
        ...
        , date_add('1900-01-01', interval TIMESTAMPDIFF(MONTH, '1900-01-01', accounts.confirmed_at) MONTH) as signup_month
      ...
      LEFT JOIN
        (
          SELECT
           DISTINCT(date_add('1900-01-01', interval TIMESTAMPDIFF(MONTH, '1900-01-01', c.created_at) MONTH)) as comment_month
          FROM comments c
          INNER JOIN discussions d on d.id = c.discussion_id
          WHERE d.topic_type IS NULL
        ) as month_list
      ON month_list.comment_month >= date_add('1900-01-01', interval TIMESTAMPDIFF(MONTH, '1900-01-01', accounts.confirmed_at) MONTH)
      LEFT JOIN
        (
          SELECT
                c.account_id
              , date_add('1900-01-01', interval TIMESTAMPDIFF(MONTH, '1900-01-01', c.created_at) MONTH) as comment_month
