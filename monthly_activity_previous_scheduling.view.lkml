view: monthly_activity_previous_scheduling {
  derived_table: {
    ...
    sql: SELECT
        ...
        , date_format(MAC.action_month, '%Y-%m') as action_month
        , MAC.account_id as account_id
        , MAC.monthly_actions
        , date_format(MAX(months_with_actions.month_with_action), '%Y-%m') as previous_action_month
      FROM
        ${monthly_activity_scheduling.SQL_TABLE_NAME} MAC
        LEFT JOIN (
          SELECT
              a1.action_month as month_with_action
            , a1.account_id as account_id
          FROM
             ${monthly_activity_scheduling.SQL_TABLE_NAME} a1
          WHERE
            a1.monthly_actions > 0
          ) months_with_actions
          on MAC.account_id = months_with_actions.account_id
          and extract(year_month from MAC.action_month) > extract(year_month from months_with_actions.month_with_action)
          WHERE MAC.action_month IS NOT NULL
      GROUP BY
        action_month, account_id, monthly_actions
 ;;
  }
