  ...
  derived_table: {
    ...
    sql: SELECT code, date_serie, COUNT(account_id) as total FROM (
          ...
          INNER JOIN (
              ...
              WHERE ((start_date IS NULL OR start_date <= CONVERT_TZ(DATE_ADD(DATE_ADD(quarter_start_date, INTERVAL +3 MONTH), INTERVAL -1 SECOND), 'America/Montreal', 'GMT')) AND (end_date IS NULL OR end_date >= CONVERT_TZ(quarter_start_date, 'America/Montreal', 'GMT'))) AND trial = 0
          ...
          INNER JOIN (
              ...
              INNER JOIN (
                  ...
                  INNER JOIN (
                      ...
                      CROSS JOIN ${date_series_quarters.SQL_TABLE_NAME}
                      WHERE MC.created_at <= CONVERT_TZ(quarter_start_date, 'America/Montreal', 'GMT') /* AND group_id = 1556 */
                  ...
                  CROSS JOIN ${date_series_quarters.SQL_TABLE_NAME}
                  WHERE MC.created_at BETWEEN CONVERT_TZ(quarter_start_date, 'America/Montreal', 'GMT') AND CONVERT_TZ(DATE_ADD(DATE_ADD(quarter_start_date, INTERVAL +3 MONTH), INTERVAL -1 SECOND), 'America/Montreal', 'GMT') /* AND MC.group_id = 1556 */
              ) QUARTER_ACTIVE ON QUARTER_ACTIVE.id = MC.id
          ) ACTIVE_ACCOUNTS_GROUPS ON ACTIVE_ACCOUNTS_GROUPS.group_id = G.id
      ) A
